...
- Registering for tax anywhere, setting up Stripe Tax, and whether the prices
  ...
  without one, Stripe shows that buyer the AUD price (`D-048`).
...
- The currency of the prices, and naming it on the session: `T-167`, `T-169`
  and `T-170`. `T-169` and `T-170` edit `docs/flows/billing.md` after this
  task, and `T-170` edits `subscribeUrl()`.
