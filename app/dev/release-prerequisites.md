...
- **Stripe**: **done in test mode** (2026-09-07) — sandbox account, `STRIPE_SECRET`/`STRIPE_WEBHOOK_SECRET` in `.env`, products and prices created, `SubscriptionPrice` rows carrying the real `price_…` ids, and the whole upgrade/downgrade cycle driven in a browser. What is still outstanding is **live mode**: the same setup again on the live account, the keys in the Laravel Cloud dashboard, and a real webhook endpoint at `/webhooks/stripe` subscribed to `checkout.session.completed` **and** `customer.subscription.*`. Plan price ids are not env vars — create the prices in Stripe, then enter each `price_…` at `/admin/pricing` as a staff owner.
    ...
    - **Opening the live account, in this order** (`D-046`, 22 September 2026). Stripe reviews the website before it activates an account, so the pages come before the account:
        ...
        5. **Settings → Adaptive Pricing on**, and **Stripe Tax threshold monitoring on**. Monitoring warns when sales in a country near its registration threshold; calculating tax stays off (`T-168`) until Qori registers somewhere.
        6. **The live prices in AUD**: Start A$55 and Pro A$139 a month (`D-046`), re-derived first if AUD/USD has moved more than 5% from 0.7126. Enter each `price_…` at `/admin/pricing` with currency `AUD` (`T-167` makes that the form's default).
        7. **Ask the accountant** whether to register for GST below A$75k — overseas suppliers add 10% GST to Qori's bills until it is registered — and, if so, whether the AUD prices include GST.
