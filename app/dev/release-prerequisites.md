...
- **Stripe**: **done in test mode** (2026-09-07) — sandbox account, `STRIPE_SECRET`/`STRIPE_WEBHOOK_SECRET` in `.env`, products and prices created, `SubscriptionPrice` rows carrying the real `price_…` ids, and the whole upgrade/downgrade cycle driven in a browser. What is still outstanding is **live mode**: the same setup again on the live account, the keys in the Laravel Cloud dashboard, and a real webhook endpoint at `/webhooks/stripe` subscribed to `checkout.session.completed` **and** `customer.subscription.*`. Plan price ids are not env vars — create the prices in Stripe, then enter each `price_…` at `/admin/pricing` as a staff owner.
    ...
    - **Opening the live account, in this order** (`D-046`, `D-047`, 22 September 2026). Stripe reviews the website before it activates an account, so the pages come before the account:
        ...
        5. **Settings → Adaptive Pricing off** (every buyer meets a fixed price, `D-047`), and **Stripe Tax threshold monitoring on**. Monitoring warns when sales in a country near its registration threshold. Calculating tax stays off (`T-168`) until Qori registers somewhere, and before it goes on, each currency option on each price needs a tax behaviour, or Stripe shows that buyer the USD price instead.
        6. **The live prices: USD as each price's currency, with your fixed prices as currency options.** Start US$39 and Pro US$99 a month. `D-047` proposes A$55/139, €35/89, £29/75, C$55/139, NZ$69/175 and S$49/129; the final figures are yours, and worth re-checking against the rates on the day. In Stripe, use **+ Add a price by currency** on each price. At `/admin/pricing`, enter each `price_…` with currency `USD` and the same fixed amounts (`T-169`), so Qori's pages quote what Stripe charges. Plan coupons are percent-off.
